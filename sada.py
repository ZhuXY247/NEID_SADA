...
class SADAModelManager:
    ...
    def get_neighbor_dataset(self, args, data, indices):
        ...
        if N_labeled > 0:
            ...
            self.train_dataloader = DataLoader(dataset, batch_size=args.train_batch_size, sampler=sampler, shuffle=False)
        else:
            ...
            for batch in tqdm(self.train_dataloader, desc="Iteration"):
                ...
                if args.view_strategy == "rtr":
                    ...
                    def apply_student(input_ids, attention_mask, special_tokens_mask, do_viz=False):
                        ...
                        if do_viz:
                            print(f"\n[SADA Augmentation Visualization - Epoch {epoch + 1}]")
                            ...
                            for t, s in zip(tokens, score_sample):
                                if t == '[PAD]': continue
                                action = "KEEP" if s > args.mask_threshold else "MASK"
                                print(f"{t:<15} | {s:.4f}       | {action}")
                            print("-" * 45 + "\n")
                        return perturbed_embeds

                    visualize_now = (nb_tr_steps == 0)
                    anchor_embeds = apply_student(anchor[0], anchor[1], do_viz=visualize_now)
                    X_an = {"inputs_embeds": anchor_embeds, "attention_mask": anchor[1], "token_type_ids": anchor[2]}
                    neighbor_embeds = apply_student(neighbor[0], neighbor[1],  do_viz=visualize_now)
    ...
    def save_results(self, args):
        ...
        var = [args.internal_dataset, args.known_cls_ratio, args.labeled_ratio, args.view_strategy, args.input_strategy, args.with_speaker, args.ratio, args.mask_threshold, args.seed]
        names = ['dataset', 'known_cls_ratio', 'labeled_ratio', 'view_strategy', 'input_strategy', 'with_speaker', 'teacher_mask_threshold', 'student_mask_threshold', 'seed']
    ...
    def visualize_Tsne(self, data, args, n_samples=2000):
        ...
        if args.save_results_path:
            file_name = f"vis_{args.internal_dataset}_L{args.labeled_ratio}_K{args.known_cls_ratio}_S{args.seed}_{args.view_strategy}.png"
...
if __name__ == '__main__':
    ...
    dir_step2 = os.path.join(args.save_model_path, "step2_internal_pretrain")
    dir_step3 = os.path.join(args.save_model_path, "step3_distillation")
    dir_step4 = os.path.join(args.save_model_path, "step4_final_sada")
    tokenizer_path = os.path.join(args.save_model_path, "tokenizer")

    for d in [dir_step2, dir_step3, dir_step4, tokenizer_path]:
        os.makedirs(d, exist_ok=True)

    ckpt_step2 = os.path.join(dir_step2, "backbone_full.pth")  # 包含 Head
    ckpt_step3_student = os.path.join(dir_step3, "student_model.pth")
    ckpt_step3_teacher = os.path.join(dir_step3, "teacher_model.pth")
    ckpt_step4 = os.path.join(dir_step4, "final_model.bin")
    ...
    ###### step1
    manager_p = None
    if os.path.exists(ckpt_step2):
        print(f"\nFound saved Step 2 model. Loading...")
        manager_p = InternalPretrainModelManager(args, data)
        manager_p.model.load_model(ckpt_step2)
        final_pretrained_model = manager_p.model
    
        print("Step 1 and 2 Skipped.")
    else:
        ...
        if args.known_cls_ratio == 0:
            ...
            print("Stage 2: Internal Pre-training SKIPPED (known_cls_ratio is 0).")
        else:
            args.disable_pretrain = False
            print('\n[Step 2] Internal Pre-training begin...')
            manager_p = InternalPretrainModelManager(args, data)
            ...
            manager_p.model.save_model(ckpt_step2)
            final_pretrained_model = manager_p.model
            print(f"[Save] Step 2 Full Model saved to {ckpt_step2}")

    ###### step3
    manager_d = NoiseManager(args, data)
    if args.view_strategy == "SADA":
        if os.path.exists(ckpt_step3_student):
            print(f"\nFound saved Student model. Loading from {ckpt_step3_student}...")
            manager_d.teacher_model.load_model(ckpt_step3_teacher)
            manager_d.student_model.load_model(ckpt_step3_student)
        else:
            ...
            print('\n[Step 3] Distillation begin...')
            manager_d.Mask_BERT_with_ratio(args, data)

            manager_d.teacher_model.save_backbone(ckpt_step3_teacher)
            manager_d.student_model.save_model(ckpt_step3_student)
            # torch.save(manager_d.tokenizer, tokenizer_path)
            print(f'[Step 3] Finished.SADA saved to {ckpt_step3_student}')
    else:
    ...
    if os.path.exists(ckpt_step4):
        print(f"\n[Check] Found saved Step 4 model at {ckpt_step4}. Loading...")
        manager_sada.model.load_model(ckpt_step4)
    else:
        if args.report_pretrain:
            method = args.method
            args.method = 'pretrain_eval'
            print('Evaluating model performance *before* SADA (after Stage 1/2)...')
            manager_sada.evaluation(args, data)
            args.method = method

        print('\n[Step 4] SADA Training begin...')
        manager_sada.train(args, data)
        print('[Step 4] Finished.')

        print('\n Saving Final Model...')
        manager_sada.model.save_backbone(ckpt_step4)
        print(f"\n Final model saved to {ckpt_step4}")
