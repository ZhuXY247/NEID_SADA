...
class TripletSequenceClassificationExplainer(SequenceClassificationExplainer):
    """
    专门处理三段式输入的解释器：Prev (Context) + Curr (Target) + Next (Context)
    适配 0-1-0 的 Segment ID 策略。
    """

    def _make_input_reference_pair(
            self, text_prev: str, text_curr: str, text_next: str
    ) -> Tuple[torch.Tensor, torch.Tensor, int]:

        # 1. 分别 Tokenize，不带特殊 Token
        t_prev_ids = self.tokenizer.encode(text_prev, add_special_tokens=False)
        t_curr_ids = self.tokenizer.encode(text_curr, add_special_tokens=False)
        t_next_ids = self.tokenizer.encode(text_next, add_special_tokens=False)

        # 2. 构建 input_ids
        # 结构: [CLS] Prev [SEP] Curr [SEP] Next [SEP]
        input_ids = (
                [self.cls_token_id]
                + t_prev_ids + [self.sep_token_id]
                + t_curr_ids + [self.sep_token_id]
                + t_next_ids + [self.sep_token_id]
        )

        # 3. 构建 ref_input_ids (Baseline)
        # 保留所有的 [CLS] 和 [SEP]，其他位置用 ref_token_id (通常是 pad) 填充
        ref_input_ids = (
                [self.cls_token_id]
                + [self.ref_token_id] * len(t_prev_ids) + [self.sep_token_id]
                + [self.ref_token_id] * len(t_curr_ids) + [self.sep_token_id]
                + [self.ref_token_id] * len(t_next_ids) + [self.sep_token_id]
        )

        return (
            torch.tensor([input_ids], device=self.device),
            torch.tensor([ref_input_ids], device=self.device),
            0,  # sep_idx 在这里不重要，因为我们会手动构建 token_type_ids
        )
    ...
    def _calculate_attributions(
        ...
        (
            ...
            self.ref_input_ids,
            self.sep_idx,
        ) = self._make_input_reference_pair(self.text_prev, self.text_curr, self.text_next)

        # 2. 生成 Position IDs (通常自动生成即可)
        (
            self.position_ids,
            self.ref_position_ids,
        ) = self._make_input_reference_position_id_pair(self.input_ids)

        # 3. 【核心修改】手动构建 0-1-0 的 Token Type IDs
        # 必须与 dataloader.py 中的逻辑完全一致
        t_prev_len = len(self.tokenizer.encode(self.text_prev, add_special_tokens=False))
        t_curr_len = len(self.tokenizer.encode(self.text_curr, add_special_tokens=False))
        t_next_len = len(self.tokenizer.encode(self.text_next, add_special_tokens=False))

        # Part 1 (Prev): [CLS] ... [SEP] -> ID 0
        seg_prev = [0] * (1 + t_prev_len + 1)
        # Part 2 (Curr): ... [SEP] -> ID 1
        seg_curr = [1] * (t_curr_len + 1)
        # Part 3 (Next): ... [SEP] -> ID 0
        seg_next = [0] * (t_next_len + 1)

        token_type_ids_list = seg_prev + seg_curr + seg_next

        self.token_type_ids = torch.tensor([token_type_ids_list], device=self.device)
        self.ref_token_type_ids = torch.tensor([token_type_ids_list], device=self.device)
    ...
    def __call__(
        ...
        if text is not None and (text_prev is None or text_curr is None or text_next is None):
            parts = text.split(delimiter)
            if len(parts) >= 3:
                text_prev = parts[0].strip()
                text_curr = parts[1].strip()
                text_next = parts[2].strip()
            else:
                text_prev = parts[0].strip() if len(parts) > 0 else ""
                text_curr = parts[1].strip() if len(parts) > 1 else ""
                text_next = parts[2].strip() if len(parts) > 2 else ""
